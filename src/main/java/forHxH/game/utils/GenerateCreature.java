...
public class GenerateCreature {
    ...
    public static Creature generateDefaultCreature(String creatureType) {
        ...
        int[] damage = generateValuesRange(DAMAGE_MIN_VALUE, DAMAGE_MAX_VALUE);
        ...
        switch (creatureType) {
            case CREATURE_TYPE_MONSTER -> {
                return new Monster(attack, damage, protection, health);
            }
            case CREATURE_TYPE_GAMER -> {
                return new Gamer(attack, damage, protection, health);
            }
